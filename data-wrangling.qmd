...
library(ggspatial)
library(terra)
library(spatialEco) #transform aspect
...
```{r}
# aspect <- project(aspect, "+proj=longlat +datum=WGS84")
# plot(aspect)
```
...
# # calculate heat load index (HLI) raster
# aspect_hli <- hli(aspect)
# 
# 
# #global(aspect, fun="notNA")
# crs(aspect) <- "NAD83"
# plot(aspect)
```
...
# elev <- rast('data/CampRoberts_spatial_data/crob_elev')
# 
# elev <- project(elev, "+proj=longlat +datum=WGS84")
# 
# elev_hli <- hli(elev)
# plot(elev_hli)
```
