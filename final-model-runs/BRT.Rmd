---
title: "Boosted Regression Tree"
author: "Briana Barajas"
date: 2024-06-03
---

## Preparation

```{r}
rm(list = ls())

# wrangling
library(tidyverse)
library(here)
library(janitor)

# raster/geospatial
library(sf)
library(tmap)
library(terra)

# modeling
library(SDMtune)
library(rsample)
library(rJava)

# source functions
source(here("R", "sdmtune-brt-final-function.R"))

```

## Plot Summary

Each plot indicates a different transect where cover board surveys were conducted, and contains it's own unique values of occurrence points. The maximum entropy model is often noted for it's capabilities even when given a small data set. Since each plot has a unique number of occurrence points, some of the analyses conducted compare plots with low vs. high numbers of points to see how the AUC was affected.

| Plot Number | Number of Occurrences |
|-------------|----------------------|
| 1           | 51                   |
| 2           | 57                   |
| 3           | 78\*                 |
| 4           | 19                   |
| 5           | 72                   |
| 6           | 39                   |
| 7           | 10\*                 |
| 8           | 21                   |


## All Variables - Iterate Through Plots 1-8

```{r, eval=FALSE}

# set data directory
rast_dir <- here("data", "raster-stacks", "full-stack")
point_dir <- here("data", "CampRoberts_spatial_data")

# create empty df
brt_all_predictions <- data.frame()


# run BRT for all 8 plots (all & reduced variables)
for (i in seq(1:2)) {
  
  # .............run both BRT models.............
  brt_res <- tune_brt(plot_number = i,
                      point_dir = point_dir,
                      rast_dir = rast_dir,
                      reduced_only = FALSE)
  
  # ..............store key results...............
  brt_p_coords <- brt_res[["p_coords"]]
  brt_a_coords <- brt_res[["a_coords"]]
  brt_pred_stack <- brt_res[["brt_pred_stack"]]
  brt_gs_full <- brt_res[["brt_gs_full"]]
  brt_gs_reduced <- brt_res[["brt_gs_reduced"]]
  
  
  # ..............select best model...............
  brt_id_full <- which.max(brt_gs_full@results$test_AUC)
  brt_best_mod_full <- combineCV(brt_gs_full@models[[brt_id_full]])
  
  brt_id_reduced <- which.max(brt_gs_reduced@results$test_AUC)
  brt_best_mod_reduced <- combineCV(brt_gs_reduced@models[[brt_id_reduced]])
  
  # .............generate predictions.............
  brt_map_full <- predict(brt_best_mod_full, data = brt_pred_stack)
  brt_map_reduced <- predict(brt_best_mod_reduced, data = brt_pred_stack)
  
  rm(brt_res, brt_id_full, brt_best_mod_full, brt_id_reduced, brt_best_mod_reduced)
  
  
  # ...........vectorize coordinates .........
  brt_p_coords <- brt_p_coords %>% vect(geom = c("x", "y"), crs = "WSG84")
  brt_a_coords <- brt_a_coords %>% vect(geom = c("x", "y"), crs = "WSG84")
  
  
  # ........isolate full predictions.........
  pres_prediction_full <- extract(brt_map_full, brt_p_coords, xy = TRUE, ID = FALSE) %>% 
    rename(prediction = lyr1) %>% 
    mutate(pres_abs = 1,
           variables = "all")
  
  abs_prediction_full <- extract(brt_map_full, brt_a_coords, xy = TRUE, ID = FALSE) %>% 
    rename(prediction = lyr1) %>% 
    mutate(pres_abs = 0,
           variables = "all")
  
  
  # .......isolate reduced predictions......
  pres_prediction_reduced <- extract(brt_map_reduced, brt_p_coords, xy = TRUE, ID = FALSE) %>% 
    rename(prediction = lyr1) %>% 
    mutate(pres_abs = 1,
           variables = "reduced")
  
  abs_prediction_reduced <- extract(brt_map_reduced, brt_a_coords, xy = TRUE, ID = FALSE) %>% 
    rename(prediction = lyr1) %>% 
    mutate(pres_abs = 0,
           variables = "all")
  
  rm(brt_p_coords, brt_a_coords, brt_map_reduced, brt_map_full)
  
  # ............combine results..........
  combined_predictions <- rbind(pres_prediction_full, abs_prediction_full, pres_prediction_reduced, abs_prediction_reduced) %>% 
    mutate(plot = i)
  
  brt_all_predictions <- rbind(brt_all_predictions, combined_predictions)
  
  rm(combined_predictions, pres_prediction_full, abs_prediction_full, pres_prediction_reduced, abs_prediction_reduced)
  
} # END for-loop


```

## Select Variables - Iterate Through Plot 1-8

```{r}
# re-write data directory
rast_dir <- here("data", "raster-stacks", "full-stack")
point_dir <- here("data", "CampRoberts_spatial_data")

# create empty df
brt_all_predictions <- data.frame()


# run BRT for all 8 plots (all & reduced variables)
for (i in seq(1:2)) {
  
  # .............run both BRT models.............
  brt_res <- tune_brt(plot_number = i,
                      point_dir = point_dir,
                      rast_dir = rast_dir,
                      reduced_only = FALSE)
  
  # ..............store key results...............
  brt_p_coords <- brt_res[["p_coords"]]
  brt_a_coords <- brt_res[["a_coords"]]
  brt_pred_stack <- brt_res[["brt_pred_stack"]]
  brt_gs_full <- brt_res[["brt_gs_full"]]
  brt_gs_reduced <- brt_res[["brt_gs_reduced"]]
  
  
  # ..............select best model...............
  brt_id_full <- which.max(brt_gs_full@results$test_AUC)
  brt_best_mod_full <- combineCV(brt_gs_full@models[[brt_id_full]])
  
  brt_id_reduced <- which.max(brt_gs_reduced@results$test_AUC)
  brt_best_mod_reduced <- combineCV(brt_gs_reduced@models[[brt_id_reduced]])
  
  # .............generate predictions.............
  brt_map_full <- predict(brt_best_mod_full, data = brt_pred_stack)
  brt_map_reduced <- predict(brt_best_mod_reduced, data = brt_pred_stack)
  
  rm(brt_res, brt_id_full, brt_best_mod_full, brt_id_reduced, brt_best_mod_reduced)
  
  
  # ...........vectorize coordinates .........
  brt_p_coords <- brt_p_coords %>% vect(geom = c("x", "y"), crs = "WSG84")
  brt_a_coords <- brt_a_coords %>% vect(geom = c("x", "y"), crs = "WSG84")
  
  
  # ........isolate full predictions.........
  pres_prediction_full <- extract(brt_map_full, brt_p_coords, xy = TRUE, ID = FALSE) %>% 
    rename(prediction = lyr1) %>% 
    mutate(pres_abs = 1,
           variables = "all")
  
  abs_prediction_full <- extract(brt_map_full, brt_a_coords, xy = TRUE, ID = FALSE) %>% 
    rename(prediction = lyr1) %>% 
    mutate(pres_abs = 0,
           variables = "all")
  
  
  # .......isolate reduced predictions......
  pres_prediction_reduced <- extract(brt_map_reduced, brt_p_coords, xy = TRUE, ID = FALSE) %>% 
    rename(prediction = lyr1) %>% 
    mutate(pres_abs = 1,
           variables = "reduced")
  
  abs_prediction_reduced <- extract(brt_map_reduced, brt_a_coords, xy = TRUE, ID = FALSE) %>% 
    rename(prediction = lyr1) %>% 
    mutate(pres_abs = 0,
           variables = "all")
  
  rm(brt_p_coords, brt_a_coords, brt_map_reduced, brt_map_full)
  
  # ............combine results..........
  combined_predictions <- rbind(pres_prediction_full, abs_prediction_full, pres_prediction_reduced, abs_prediction_reduced) %>% 
    mutate(plot = i)
  
  brt_all_predictions <- rbind(brt_all_predictions, combined_predictions)
  
  rm(combined_predictions, pres_prediction_full, abs_prediction_full, pres_prediction_reduced, abs_prediction_reduced)
  
} # END for-loop

```






------------------------
#        END !!
------------------------ 

## Test BRT Function
```{r, eval=FALSE}
# run model
brt_res <- tune_brt(plot_number = 1,
                    point_dir = point_dir,
                    rast_dir = rast_dir,
                    reduced_only = FALSE)

# pull models from results
brt_p_coords <- brt_res[["p_coords"]]
brt_test_obj <- brt_res[["brt_test"]]
brt_pred_stack <- brt_res[["brt_pred_stack"]]
brt_gs_full <- brt_res[["brt_gs_full"]]
brt_gs_reduced <- brt_res[["brt_gs_reduced"]]

# best model predictions for FULL model 
brt_id_full <- which.max(brt_gs_full@results$test_AUC)
brt_best_mod_full <- combineCV(brt_gs_full@models[[brt_id_full]])
brt_map_full <- predict(brt_best_mod_full, data = brt_pred_stack)

# best modl predictions for REDUCED model
brt_id_reduced <- which.max(brt_gs_reduced@results$test_AUC)
brt_best_mod_reduced <- combineCV(brt_gs_reduced@models[[brt_id_reduced]])
brt_map_reduced <- predict(brt_best_mod_reduced, data = brt_pred_stack)

# plot ROC results
plotROC(brt_best_mod_full, test = brt_test_obj) + labs(title = "All Environmental Variables")
plotROC(brt_best_mod_reduced, test = brt_test_obj) + labs(title = "Reduced Environmental Variables")

# tm_shape(brt_map) +
#   tm_raster(midpoint = 0) +
#   tm_shape(points) +
#   tm_dots(size = 0.1) +
#   tm_layout(legend.position = c(1, 0.2))
```

## Extract Values
```{r, eval=FALSE}
# extract absence prediction values
abs_points <- pres_abs_points %>% 
  filter(basp_pa == 0) %>% 
  select(longitude, latitude) %>% 
  vect(geom=c("longitude", "latitude"), crs="WSG84")

abs_predicted <- extract(brt_map, abs_points, xy = TRUE, ID = FALSE) %>% 
  mutate(basp_pa = 0) %>% 
  rename(prediction = lyr1)

# extract presence prediction values
pres_points <- pres_abs_points %>% 
  filter(basp_pa == 1) %>% 
  select(longitude, latitude) %>% 
  vect(geom=c("longitude", "latitude"), crs="WSG84")

pres_predicted <- extract(brt_map, pres_points, xy = TRUE, ID = FALSE) %>% 
  mutate(basp_pa = 1) %>% 
  rename(prediction = lyr1)

# combine
abs_predicted %>% 
  rbind(pres_predicted) %>% 
  mutate(model = "BRT reduced")

```

