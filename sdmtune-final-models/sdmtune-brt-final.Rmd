...
param_tune <- list(distribution = "gaussian", #for continuous output variables
...
best_loc <- which.max(gs@results$test_AUC)

# retrain model with best parameters for final prediction
best_model <- combineCV(gs@models[[best_loc]])

```
__NOTE:__ The best perdorming values of shrinkage are actually larger, which is interesting. Typically the smaller the value of shrinkage, the higher the model accurace.

### Plots
```{r}
plotROC(best_model, test = test)
```
