...
tune_brt(1, data_dir, output_dir)
```

```{r}
# view top performing models
head(brt_gs@results[order(-brt_gs@results$test_AUC), ])  

# view parameters for best model
brt_gs@models[[1]]

# isolate index location for the best parameters
best_locB <- which.max(brt_gs@results$test_AUC)

# retrain model with best parameters for final prediction
best_modB <- combineCV(brt_gs@models[[best_locB]])
```

```{r}
# plot AUC curves 
plotROC(best_modB, test = brt_test)
