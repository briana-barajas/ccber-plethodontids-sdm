...
tune_maxent(1, data_dir, output_dir)
```

Isolate final best-fit model
```{r}
# view top performing models
head(gs_results@results[order(-gs_results@results$test_AUC), ])  

# view parameters for best model
#gs_results@models[[1]]

# isolate index location for the best parameters
best_loc <- which.max(gs_results@results$test_AUC)

# retrain model with best parameters for final prediction
best_model <- combineCV(gs_results@models[[best_loc]])
```
